...
import { useGenerateShareLink, useTrips } from "@/hooks/useTrips";
...
export default function TripDetailsPage() {
  ...
  const shareLink = useGenerateShareLink(async (result) => {
  ...
  const freeMemberLimitReached = !isPro && memberCount !== null && memberCount >= 5;

  const requestUpgrade = (reason) => {
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-2">
          ...
          {isOwner && (
            <Button
              ...
              onClick={() => {
                if (!isPro) {
                  requestUpgrade("Share links are available on Pro.");
                  return;
                }
                shareLink.mutate(tripId);
              }}
      ...
      <Tabs defaultValue={state?.activeTab ?? "members"} className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger
            value="comments"
            onClick={() => navigate(`/trips/${tripId}/comments`)}
          >
            Comments
          </TabsTrigger>
          <TabsTrigger
                ...
                <Button
                  ...
                  onClick={() => {
                    if (freeMemberLimitReached) {
                      requestUpgrade("Free trips can include up to 5 members.");
                      return;
                    }
